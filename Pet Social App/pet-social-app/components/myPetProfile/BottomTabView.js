...
const BottomTabView = ({petOfUser , currentUserLogin}) => {
    ...
    const [postOfPet, setpostOfPet] = useState(null)
        ...
        useEffect(() => { 
            ...
            const unsubscribe = onSnapshot(postsRef, orderBy('createdAt','desc'), (snapshot) => {
    ...
    const handlPress = (post) => {
        navigation.navigate('MyDetailPostScreen',{post});
    }
    const handlPressVideo = (video) => {
        navigation.navigate('MyDetailVideoScreen', {video});
    }
    const Posts = () => {
        return (
            ...
            <View style={styles.postView}>
                {postOfPet && postOfPet.map((post,index)=>(
                    <TouchableOpacity onPress={() => handlPress(post)} key={index}>
                        <View 
                            style={{
